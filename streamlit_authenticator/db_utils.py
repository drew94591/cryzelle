...
def get_default_active_user_wallet_by_phone(phone):
    ...
    connection = engine.connect()
    try:
        stmt = text("SELECT wallet.id, wallet_nickname, wallet_link, first_name, last_name FROM users INNER JOIN wallet_accounts wallet ON users.id = wallet.user_id INNER JOIN user_profiles up ON wallet.user_id = up.user_id WHERE mobile_number = :cp and is_default is true and is_active is true")
        result = connection.execute(stmt, cp = phone).one_or_none()
    finally:
        connection.close()
    return result
...
def get_default_active_user_wallet_by_email(email):
    ...
    connection = engine.connect()
    try:
        stmt = text("SELECT wallet.id, wallet_nickname, wallet_link FROM users JOIN wallet_accounts wallet ON users.id = wallet.user_id JOIN user_profiles up ON users.id = up.user_id WHERE email_address = :email_address AND is_default is true AND is_active is true")
